...
flow=FordFulkerson(N*2)
for _ in range(M):
    a,b=map(int, input().split())
    a-=1
    b-=1
    flow.add_edge(a,b+N,pow(10,12))
    flow.add_edge(b,a+N,pow(10,12))
c_array=list(map(int, input().split()))
for i in range(1,N-1):
    flow.add_edge(i+N,i,c_array[i])
ret=flow.flow(0,2*N-1)
print(ret)
cuts=flow.min_cut(0)
rets=[]
for i in range(N):
    if cuts[i+N] and not cuts[i]:
        rets.append(i+1)
print(len(rets))
print(*rets)
