...
sorted_array=SortedList([-1,-1,array_with_index[-1][1],N,N])
ret=0
for i in range(N-2,-1,-1):
    a,a_index=array_with_index[i]
    k=sorted_array.bisect(a_index)
    c=0
    c+=(sorted_array[k+1]-sorted_array[k])*(a_index-sorted_array[k-1])
    c+=(sorted_array[k-1]-sorted_array[k-2])*(sorted_array[k]-a_index)
    ret+=c*a
    sorted_array.add(a_index)
print(ret)
