...
while targets:
    ...
    if status[t]==0:
        ...
        #print(t,status,meds, list(routes))
        if len(edges[t])==1 and depth[t]!=0:
            l=len(routes)
            if l%2==0:
                meds[t]=(routes[l//2]+routes[l//2-1])//2
            else:
                meds[t]=routes[l//2]
        else:
            mini=pow(10,10)
            ...
            for u in edges[t]:
                if depth[u]==depth[t]+1:
                    mini=min(mini,meds[u])
                    maxi=max(maxi,meds[u])
            if depth[t]%2==0:
                meds[t]=maxi
            else:
                meds[t]=mini
        routes.remove(array[t])
print(meds[0])
