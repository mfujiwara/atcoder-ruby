...
from operator import add, eq, index, ne, gt, ge, lt, le, iadd
...
for _ in range(T):
    ...
    for i in range(N):
        ...
        if l==r or k==N:
            ...
            ret+=r
            lefts.append((l-r,k-1))
        else:
            ret+=l
            rights.append((r-l,k))
    lefts.sort()
    indexes=SortedList([i for i in range(len(lefts))])
    while lefts:
        p,k=lefts.pop()
        i=indexes.bisect_right(k)
        if i!=0:
            ret+=p
            indexes.pop(i-1)
    rights.sort()
    indexes=SortedList([i for i in range(N-len(rights),N)])
    while rights:
        p,k=rights.pop()
        i=indexes.bisect_left(k)
        if i!=len(indexes):
            ret+=p
            indexes.pop(i)
    print(ret)
