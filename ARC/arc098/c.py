...
ret=sorted_array[Q-1][0]-sorted_array[0][0]
dead=SortedList([-1,N])
for i in range(N):
    enable=[]
    for j in range(len(dead)-1):
        if dead[j]+1+K<=dead[j+1]:
            sub=array[dead[j]+1:dead[j+1]]
            if K==1:
                enable+=sub
            else:
                sub.sort()
                enable+=sub[:min(len(sub)-K+1,Q)]
    if len(enable)<Q:
        break
    enable.sort()    
    r=enable[Q-1]-enable[0]
    ret=min(ret,r)
    dead.add(sorted_array[i][1])
print(ret)
